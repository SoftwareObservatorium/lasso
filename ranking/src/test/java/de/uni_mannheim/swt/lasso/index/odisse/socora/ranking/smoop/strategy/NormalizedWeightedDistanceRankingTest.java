 ...
 *
 */
public class NormalizedWeightedDistanceRankingTest {
    
    private static final Logger LOG = LoggerFactory
            .getLogger(NormalizedWeightedDistanceRankingTest.class);

    private SmoopRanking smoopRanking;

    public NormalizedWeightedDistanceRankingTest() throws IOException {
        smoopRanking = new SmoopRanking();
    }

    @Test
    public void test_max() throws IOException {
        NormalizedWeightedDistanceRanking rankingStrategy = new NormalizedWeightedDistanceRanking(
                smoopRanking);
        ...
        for (int i = 1; i <= candidateRanking.getCandidateItems().length; i++) {
            assertEquals(i, candidateRanking.getCandidateItems()[i - 1]
                    .getRanking().get(NormalizedWeightedDistanceRanking.ID)
                    .intValue());
            
            CandidateItem candidate = candidateRanking.getCandidateItems()[i - 1];
            LOG.debug("Reference Point Distance Penalty score for candidate " + candidate.getId() + " = " + candidate.getRanking().get("rpDistanceScore"));
        }
    ...
    public void test_min() throws IOException {
        ...
        Criterion[] criteria = new Criterion[] { Tester.toCriterion("metric1",
                Criterion.OBJECTIVE_MIN, 1d, 1d), Tester.toCriterion("metric2",
                        Criterion.OBJECTIVE_MIN, 1d, 1d) };
