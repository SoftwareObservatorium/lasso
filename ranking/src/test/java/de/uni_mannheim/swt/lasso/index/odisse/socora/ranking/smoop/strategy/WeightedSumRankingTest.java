 ...
 *
 */
public class WeightedSumRankingTest {

    private SmoopRanking smoopRanking;

    public WeightedSumRankingTest() throws IOException {
        smoopRanking = new SmoopRanking();
    }

    @Test
    public void test() throws IOException {
        WeightedSumRanking rankingStrategy = new WeightedSumRanking(
                smoopRanking);
        ...
        for (int i = 1; i < candidateRanking.getCandidateItems().length; i++) {
            assertEquals(i, candidateRanking.getCandidateItems()[i - 1]
                    .getRanking().get(WeightedSumRanking.ID).intValue());
        }
    }
}
