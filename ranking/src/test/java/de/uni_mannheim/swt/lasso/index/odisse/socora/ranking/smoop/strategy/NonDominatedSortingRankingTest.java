 ...
 *
 */
public class NonDominatedSortingRankingTest {

    private static final Logger LOG = LoggerFactory
            .getLogger(NonDominatedSortingRankingTest.class);

    private SmoopRanking smoopRanking;

    public NonDominatedSortingRankingTest() throws IOException {
        smoopRanking = new SmoopRanking();
    }

    @Test
    public void test_max() throws IOException {
        NonDominatedSortingRanking rankingStrategy = new NonDominatedSortingRanking(
                smoopRanking);
        // SET TO TRUE!
        ...
        for (int i = 1; i <= candidateRanking.getCandidateItems().length; i++) {
            assertEquals(i, RankingUtils.getRank(
                    ...
                    NonDominatedSortingRanking.ID, false));
            ...
            LOG.debug("Rank SO vs PO: "
                    + RankingUtils.getRank(
                            candidateRanking.getCandidateItems()[i - 1],
                            NonDominatedSortingRanking.ID,
                            false)
                    + " "
                    + RankingUtils.getRank(
                            candidateRanking.getCandidateItems()[i - 1],
                            NonDominatedSortingRanking.ID, true));
        }
        ...
        for (int i = 1; i <= candidateRanking.getCandidateItems().length; i++) {
            assertEquals(i,
                    candidateRanking.getCandidateItems()[i - 1].getRanking()
                            .get(NonDominatedSortingRanking.ID)
                            .intValue());
