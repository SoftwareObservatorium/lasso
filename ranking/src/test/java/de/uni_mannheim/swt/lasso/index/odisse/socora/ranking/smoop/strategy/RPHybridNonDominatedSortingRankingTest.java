 ...
 *
 */
public class RPHybridNonDominatedSortingRankingTest {

    private static final Logger LOG = LoggerFactory
            .getLogger(RPHybridNonDominatedSortingRankingTest.class);

    private SmoopRanking smoopRanking;

    public RPHybridNonDominatedSortingRankingTest() throws IOException {
        smoopRanking = new SmoopRanking();
    }

    @Test
    public void test_max() throws IOException {
        RPHybridNonDominatedSortingRanking rankingStrategy = new RPHybridNonDominatedSortingRanking(
                smoopRanking);
        // SET TO TRUE!
        ...
        for (int i = 1; i <= candidateRanking.getCandidateItems().length; i++) {
            assertEquals(i, RankingUtils.getRank(
                    ...
                    RPHybridNonDominatedSortingRanking.ID, false));
            ...
            LOG.debug("Rank SO vs PO: "
                    + RankingUtils.getRank(
                            candidateRanking.getCandidateItems()[i - 1],
                            RPHybridNonDominatedSortingRanking.ID,
                            false)
                    + " "
                    + RankingUtils.getRank(
                            candidateRanking.getCandidateItems()[i - 1],
                            RPHybridNonDominatedSortingRanking.ID, true));
        }
        ...
        for (int i = 1; i <= candidateRanking.getCandidateItems().length; i++) {
            assertEquals(i,
                    candidateRanking.getCandidateItems()[i - 1].getRanking()
                            .get(RPHybridNonDominatedSortingRanking.ID)
                            .intValue());
