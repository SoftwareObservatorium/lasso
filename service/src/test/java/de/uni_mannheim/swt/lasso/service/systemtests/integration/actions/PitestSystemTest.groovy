...
/**
 * Demonstrates Pitest action
 *
 * @author mkessel
 */
class PitestSystemTest extends AbstractGroovySystemTest {
    ...
    void test_junit_class_stack() throws IOException, DataSourceNotFoundException {
        ...
        study(name: 'Select') {
            ...
            action(name:"pitest",type:'Pitest') {
                ...
                whenAbstractionsReady {
                    ...
                    Table table = queryReport("select * from PitestReport where action = '${getName()}' and abstraction = '${stack.name}'")
                    log(table?.printAll())
                    
                    Table mutants = queryReport("select * from PitMutation where action = '${getName()}' and abstraction = '${stack.name}'")
                    log(mutants?.printAll())
                }
            }
        }
        '''
        ...
        verifyAbstraction(lslExecutionContext, 'pitest', 'Stack', 1)
        ...
        Table mutations = lslExecutionContext.getReportOperations().select(scriptUnderTest.getExecutionId(),
                "select * from PitMutation where action = 'pitest' and abstraction = 'Stack'")
        println(mutations.printAll())
        assert mutations.size() == 7
    }
    ...
    void test_junit_class_stack_TIMEOUT() throws IOException, DataSourceNotFoundException {
        ...
        study(name: 'Select') {
            ...
            action(name:"pitest",type:'Pitest') {
                timeoutClientProcess = 3 // TIMEOUT UNDER TEST
        ...
        verifyAbstraction(lslExecutionContext, 'select', 'Stack', 1)
        verifyAbstraction(lslExecutionContext, 'test', 'Stack', 1)
        verifyAbstraction(lslExecutionContext, 'pitest', 'Stack', 0)
...
//        assert mutations.size() == 7
    ...
    void test_arena_class_stack_PARALLEL() throws IOException, DataSourceNotFoundException {
        ...
        verifyAbstraction(lslExecutionContext, 'pitest', 'Stack', 3)
