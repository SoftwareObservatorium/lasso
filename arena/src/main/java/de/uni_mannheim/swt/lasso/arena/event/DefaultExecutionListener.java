...
import de.uni_mannheim.swt.lasso.arena.classloader.statistics.LoaderObservation;
 ...
 */
public class DefaultExecutionListener implements ArenaExecutionListener {

    private final Container container;

    public DefaultExecutionListener(Container container) {
    ...
    public void onAfterExecution(SequenceExecutionRecords results) {
        try {
            LoaderObservation observation = new LoaderObservation(container);
            results.addObservation("loader", observation);
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
