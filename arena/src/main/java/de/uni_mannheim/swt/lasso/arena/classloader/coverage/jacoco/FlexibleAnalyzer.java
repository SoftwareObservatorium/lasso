...
import org.jacoco.core.internal.analysis.FlexibleClassAnalyzer;
...
/**
 * copy of {@link org.jacoco.core.analysis.Analyzer} that supports setting custom filters.
 */
public class FlexibleAnalyzer {
     ...
     * @param filter
     */
    public FlexibleAnalyzer(final ExecutionDataStore executionData,
                            final ICoverageVisitor coverageVisitor, IFilter filter) {
    ...
    private ClassVisitor createAnalyzingVisitor(final long classid,
        ...
        final ClassCoverageImpl coverage = new ClassCoverageImpl(className,
                classid, noMatch);
        final FlexibleClassAnalyzer analyzer = new FlexibleClassAnalyzer(coverage, probes,
                stringPool, filter) {
            @Override
    ...
    private IOException analyzerError(final String location,
        ...
        final IOException ex = new IOException(
                String.format("Error while analyzing %s with JaCoCo %s/%s.",
                        location, JaCoCo.VERSION, "custom"));
        ex.initCause(cause);
        return ex;
    }
