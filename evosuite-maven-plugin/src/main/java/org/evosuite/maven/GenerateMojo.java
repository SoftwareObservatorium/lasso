...
public class GenerateMojo extends AbstractMojo {
	...
	@Parameter( property = "lassoRepetitions", defaultValue = "1" )
	private int lassoRepetitions;
	...
	//
	private CustomSpawnProcessKeepAliveChecker customSpawnProcessKeepAliveChecker;
	...
	public void execute() throws MojoExecutionException, MojoFailureException{
		...
		// repetitions
		for(int i = 0; i < lassoRepetitions; i++) {
			getLog().info("Lasso Repetition: " + (i+1));

			runEvoSuiteOnSeparatedProcess(target, cp, basedir.getAbsolutePath(), i);

			// check if tests were generated
//			List<File> files = FileUtils.scan(new File(this.project.getBasedir(), "evosuite-tests"), this.includes, this.excludes);
//
//			if(CollectionUtils.isEmpty(files)) {
//				getLog().error("No tests found in Repetition: " + (i+1));
//				throw new MojoFailureException("No tests were generated");
//			}

			// TODO check if notGeneratedTests method exists .. useful?

//			if(lassoRepetitions > 1) {
//				// rename generated test classes to avoid loss
//				List<File> files = FileUtils.scan(new File(this.project.getBasedir(), "evosuite-tests"), this.includes, this.excludes);
//
//				if(!CollectionUtil.isNullOrEmpty(files)) {
//					for(File testClass : files) {
//						// only new ones
//						if(!testClass.getName().contains("_")) {
//							getLog().info("Generated test class: " + testClass);
//
//							File renamedClass = new File(testClass.getParentFile(), String.format("%s_%s", i, testClass.getName()));
//							testClass.renameTo(renamedClass);
//
//							getLog().info("Renaming to test class: " + renamedClass);
//						}
//					}
//				}
//			}
	...
	private void runEvoSuiteOnSeparatedProcess(String target, String cp, String dir, int repetition) throws MojoFailureException {
...
//		}
		...
		//-- lasso
		boolean lasso = lassoClass != null && !lassoClass.isEmpty();
		if(lasso) {
//			params.add("-projectCP");
//			params.add(cp);

			//-Dsearch_budget=60 -Dstopping_condition=MaxTime etc.
			if(extraArgs!=null && !extraArgs.isEmpty()){
				//note this does not for properly for parameters with strings using spaces
				String[] tokens = extraArgs.split(" ");
				for(String token : tokens){
					...
					if(!token.startsWith("-D")){
						...
						// add
						if(repetition >= 0 && StringUtils.startsWithIgnoreCase(token, "-Djunit_suffix=")) {
							String[] parts = StringUtils.split(token, "=");
							params.add(String.format("-Djunit_suffix=_%s_%s", repetition, parts[1]));

							// add label to track repetition
							params.add(String.format("-Dconfiguration_id=%s", repetition));

							getLog().info("Adjusting junit suffix for repetition " + repetition);
						} else {
							params.add(token);
						}
					}
			...
			params.add("-class");
			params.add(lassoClass);
			//
			getLog().info(String.format("Running Lasso class '%s' with classpath '%s'", lassoClass, cp));
		} else {
			params.add("-continuous");
			params.add("execute");
		...
		// init alive checker
		customSpawnProcessKeepAliveChecker = new CustomSpawnProcessKeepAliveChecker();

		int port;
		if(spawnManagerPort != null) {
			//SpawnProcessKeepAliveChecker.getInstance().registerToRemoteServerAndDieIfFails(spawnManagerPort);
			customSpawnProcessKeepAliveChecker.registerToRemoteServerAndDieIfFails(spawnManagerPort);

			port = spawnManagerPort;
		} else {
			port = customSpawnProcessKeepAliveChecker.startServer();
		}
		params.add("-Dspawn_process_manager_port=" + port);
		...
		if(spawnManagerPort != null) {
			customSpawnProcessKeepAliveChecker.unRegister();
		} else {
			customSpawnProcessKeepAliveChecker.stopServer();
		}
