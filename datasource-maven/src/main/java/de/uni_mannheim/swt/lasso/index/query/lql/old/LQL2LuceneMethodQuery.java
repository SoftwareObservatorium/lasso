...
/**
 * Default candidate index query (supports free text searches as well as MQL
 * interface signatures searches).
 * 
 * @author Marcus Kessel
 * 
 */
public class LQL2LuceneMethodQuery implements LQLLuceneStrategy {

    private static final Logger LOG = LoggerFactory.getLogger(LQL2LuceneMethodQuery.class);

    /**
     * Default field boosts (basically taken and adapted from old merobase
     * heuristics).
     */
    protected static final Map<String, Float> FIELD_BOOSTS = new HashMap<String, Float>();
    static {
    ...
    /**
     * Class name boost for MQL Lucene {@link Query}s
     */
    protected static final float MQL_CLASS_NAME_BOOST = 2.5f;
    ...
    public LQL2LuceneMethodQuery(SearchOptions searchOptions) {
        //
    ...
    protected Query createLuceneKeywordsAndConstraintsQuery(String searchQuery) throws ParseException {
        ...
        MultiFieldQueryParser qp = new MultiFieldQueryParser(
                new String[] { "packagename_fq", "method_fq", "content" }, analyzer, LQL2LuceneMethodQuery.FIELD_BOOSTS);
...
//                TermQuery classNameTerm = new TermQuery(new Term("name_fq", name));
//                classNameTerm.setBoost(MQL_CLASS_NAME_BOOST);
//
//                interfaceSignatureQuery.add(classNameTerm, Occur.SHOULD);
//            });
//        } else {
//            // default
//
...
//        }
        ...
        // TermQuery interfaceNameTerm = new TermQuery(new Term("interface",
        // className));
        // interfaceNameTerm.setBoost(MQL_CLASS_NAME_BOOST);
        // TermQuery superclassNameTerm = new TermQuery(new Term("superclass",
        // className));
        // superclassNameTerm.setBoost(MQL_CLASS_NAME_BOOST);


        // interfaceSignatureQuery.add(interfaceNameTerm,
        // BooleanClause.Occur.SHOULD);
        // interfaceSignatureQuery.add(superclassNameTerm,
        // BooleanClause.Occur.SHOULD);

        // // constructors
        // if (ArrayUtils.isNotEmpty(constructorSignatures)) {
        // for (Signature constructorSignature : constructorSignatures) {
        // // no params?
        // if (CollectionUtils.isEmpty(constructorSignature
        // .getParameterTypes())) {
        // // no params? no need to create term query
        // continue;
        // }
        //
        // // since constructorName is determined by className only care
        // // about the signature
        // TermQuery constructorSignatureQuery = new TermQuery(
        // new Term(
        // "constructorParameterOrderedSyntax",
        // constructorSignature
        // .toStringConstructorParameterOrderedSyntax(false)));
        // constructorSignatureQuery.setBoost(2.0f);
        // //
        // interfaceSignatureQuery.add(constructorSignatureQuery,
        // BooleanClause.Occur.SHOULD);
        // }
        // }
        ...
        if (ArrayUtils.isNotEmpty(methodSignatures)) {
            ...
            for (MethodSignature methodSignature : methodSignatures) {
                ...
                TermQuery methodSignatureParamsOrderedSyntaxQuery = new TermQuery(
                        new Term(toMethodField("methodSignatureParamsOrderedSyntaxKeywordsFq_ssig"),
                ...
                aq.add(new TermQuery(new Term(toMethodField("methodSignatureParamsOrderedKeywordsFq_ssig"),
                        methodSignature.toStringParameterOrdered(isFullyQualified(), true))), Occur.SHOULD);

                // method
                TermQuery tq = new TermQuery(new Term("method_fq", methodSignature.getMethodName()));
                //tq.setBoost(1.0f);
...
//                if(searchOptions.isMqlIncludeInheritedMethods()) {
//                    TermQuery inheritedMethodSignatureParamsOrderedSyntaxQuery = new TermQuery(
//                            new Term("inheritedmethodSignatureParamsOrderedSyntaxKeywordsFq_sigs",
//                                    methodSignature.toStringParameterOrderedSyntax(true, true)));
...
//
//                    TermQuery inheritedMethodSignatureParamsOrderedKeywords = new TermQuery(new Term("inheritedmethodSignatureParamsOrderedKeywordsFq_sigs",
//                            methodSignature.toStringParameterOrdered(true, true)));
//
//                    if(methodSignature.getPermutations() != null) {
//                        for(MethodSignature alt : methodSignature.getPermutations()) {
//                            TermQuery altMethodSignatureParamsOrderedSyntaxQuery = new TermQuery(
//                                    new Term("inheritedmethodSignatureParamsOrderedKeywordsFq_sigs",
...
//        }
        ...
        //q.add(new BoostQuery(classNameTerm, MQL_CLASS_NAME_BOOST), Occur.SHOULD);
...
//                    // methodSignatureParamsOrdered
//                    q.add(new TermQuery(new Term("methodSignatureParamsOrderedKeywordsFq_ssig",
//                            constructor.toStringParameterOrdered(true, true))), Occur.SHOULD);
//                }
//            }
//        }
    ...
    /**
     * @param keywordsAndConstraints
     *            the keywordsAndConstraints to set
     */
    @Override
