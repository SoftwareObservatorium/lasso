 ...
 */
public class CompoundFilter implements SignatureMatchFilter {

    private final List<SignatureMatchFilter> filters;

    public CompoundFilter(List<SignatureMatchFilter> filters) {
        this.filters = filters;
    }

    @Override
    public boolean accept(SignatureMatch signatureMatch, boolean constructor) {
        for (SignatureMatchFilter filter : filters) {
            if (!filter.accept(signatureMatch, constructor)) {
                return false;
            }
        }
